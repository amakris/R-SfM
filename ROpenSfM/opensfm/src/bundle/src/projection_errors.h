...
class ReprojectionError2DRefraction : public ReprojectionError {
 public:
 ReprojectionError2DRefraction(const ProjectionType& type, const Vec2d& observed,
                    double std_deviation)
  ...
  bool operator()(const T* const camera, const T* const shot,
    ...
    //std::cout << "ReprojectionError2DRefraction::shot coords:" << shot[BA_SHOT_TX] << ", " << shot[BA_SHOT_TY] << ", " << shot[BA_SHOT_TZ] << std::endl;
    Vec3<T> E;
    E[0] = shot[BA_SHOT_TX];
    E[1] = shot[BA_SHOT_TY];
    E[2] = shot[BA_SHOT_TZ];
    ...
    T surface_point[3];
    // If CalcR fails to locate R calculate residual using S.
    if (rv) {      
      surface_point[0] = R[0];
      surface_point[1] = R[1];
      surface_point[2] = R[2];
    ...
    T camera_point[3];
    WorldToCameraCoordinates(shot, surface_point, camera_point);

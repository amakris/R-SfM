...
struct PositionPriorError {
  ...
  template <typename T>
  bool operator()(const T* const shot, T* residuals) const {
    //std::cout << "PositionPriorError::shot coords:" << shot[BA_SHOT_TX] << ", " << shot[BA_SHOT_TY] << ", " << shot[BA_SHOT_TZ] << std::endl;
    //std::cout << "PositionPriorError::shot position_prior_:" << position_prior_[0] << ", " << position_prior_[1] << ", " << position_prior_[2] << std::endl;

...
py::tuple BAHelpers::BundleLocal(
  ...
  ba.SetUseRefraction(
      config["bundle_refraction"].cast<bool>());
  ...
  for (auto* shot : interior) {
    ...
    shot->GetPoseRef().SetFromWorldToCamera(s.GetRotation(),
                                            s.GetTranslation());
  }
...
py::dict BAHelpers::Bundle(
  ...
  for (const auto& shot_pair : map.GetShots()) {
    ...
    const auto& pose = shot.GetPose();
    bool fix_shot = config["bundle_fix_shot"].cast<bool>();
    ...
    if (config["bundle_use_gimbal"].cast<bool>()) {
      const Vec3d r = shot.GetShotMeasurements().gimbal_rotation_.Value();
      ba.AddRotationPrior(shot.id_,r[0],r[1],r[2],
                          shot.GetShotMeasurements().gimbal_accuracy_.Value());
    }
  ...
  for (auto& shot : map.GetShots()) {
    ...
    shot.second.GetPoseRef().SetFromWorldToCamera(s.GetRotation(),
                                                  s.GetTranslation());
  }

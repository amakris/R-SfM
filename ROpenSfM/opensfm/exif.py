...
class EXIF:
    ...
    def extract_camera_rotation(self):
        y = None
        p = None
        r = None
        if self.has_dji_gimbalrot():
            y,p,r = self.extract_dji_gimbalrot()
        elif self.has_dji_flightrot():
            y,p,r = self.extract_dji_flighrot()
        d = {}
        if y is not None:
            d['yaw'] = y
            d['pitch'] = p
    ...
    def has_dji_gimbalrot(self):
        return self.has_xmp() and \
                '@drone-dji:GimbalRollDegree' in self.xmp[0] and \
                '@drone-dji:GimbalYawDegree' in self.xmp[0] and \
                '@drone-dji:GimbalPitchDegree' in self.xmp[0] 

    def has_dji_flightrot(self):
            return self.has_xmp() and \
                    '@drone-dji:FlightRollDegree' in self.xmp[0] and \
                    '@drone-dji:FlightYawDegree' in self.xmp[0] and \
                    '@drone-dji:FlightPitchDegree' in self.xmp[0] 

    def extract_dji_gimbalrot(self):
        r = float(self.xmp[0]["@drone-dji:GimbalRollDegree"])
        y = float(self.xmp[0]["@drone-dji:GimbalYawDegree"])
        p = float(self.xmp[0]["@drone-dji:GimbalPitchDegree"])
        return r,p,y

    def extract_dji_flightrot(self):    
        r = float(self.xmp[0]["@drone-dji:FlightRollDegree"])
        y = float(self.xmp[0]["@drone-dji:FlightYawDegree"])
        p = float(self.xmp[0]["@drone-dji:FlightPitchDegree"])
        return r,p,y
    ...
    def extract_exif(self):
        ...
        camera_rotation = self.extract_camera_rotation()
